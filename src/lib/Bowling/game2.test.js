...
test('前一轮没有投中10个，第一次投球打中8个： 状态由NONE变为ONCE, 总分36', () => {
    game.throw(6)
    game.throw(3)
...
test('前一轮没有投中10个，第一次投球打中10个： 状态由NONE变为STRIKE, 总分38，轮数+1', () => {
    game.throw(6)
    game.throw(3)
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由SRARE变为ONCE, 总分46', () => {
    game.throw(2)
    game.throw(8)   
    expect(game.getState()).toBe(SRARE)
    game.throw(8)   
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由SRARE变为ONCE, 总分46； 第二次投中2个， 状态由ONCE变为SRARE, 总分48,轮数+1', () => {
    game.throw(2)
    game.throw(8)
    const prevRound = game.getRound()
    expect(game.getState()).toBe(SRARE)
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由SRARE变为ONCE, 总分54； 第二次投中1个， 状态由ONCE变为NONE, 总分47,轮数+1', () => {
    game.throw(2)
    game.throw(8)
    const prevRound = game.getRound()
    expect(game.getState()).toBe(SRARE)
...
test('前一轮两投投中10个，这一轮第一次投球打中10个： 状态由SRARE变为STRIKE, 总分50,轮数+1', () => {
    game.throw(2)
    game.throw(8)
    const prevRound = game.getRound()
    expect(game.getState()).toBe(SRARE)
