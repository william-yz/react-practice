...
test('前一轮没有投中10个，第一次投球打中8个： 状态由NONE变为ONCE, 总分26', () => {
    game.throw(6)
    game.throw(3)
    expect(game.getState()).toBe(NONE)
    game.throw(8)
...
test('前一轮没有投中10个，第一次投球打中8个：状态由NONE变为ONCE, 总分26; 第二次投中1个: 状态由ONCE变为NONE, 总分27', () => {
    game.throw(6)
    game.throw(3)
    game.throw(8)
    expect(game.getState()).toBe(ONCE)
...
test('前一轮没有投中10个，第一次投球打中8个：状态由NONE变为ONCE, 总分26; 第二次投中2个: 状态由ONCE变为SRARE, 总分28, 局数+1', () => {
    game.throw(6)
    game.throw(3)
...
test('前一轮没有投中10个，第一次投球打中10个： 状态由NONE变为STRIKE, 总分28，轮数+1', () => {
    game.throw(6)
    game.throw(3)
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由SRARE变为ONCE, 总分35', () => {
    game.throw(2)
    game.throw(8)  
    expect(game.getState()).toBe(SRARE)
    game.throw(8) 
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由SRARE变为ONCE, 总分35； 第二次投中2个， 状态由ONCE变为SRARE, 总分37,轮数+1', () => {
    game.throw(2)
    game.throw(8)
    const prevRound = game.getRound()
    expect(game.getState()).toBe(SRARE)
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由SRARE变为ONCE, 总分35； 第二次投中1个， 状态由ONCE变为NONE, 总分36,轮数+1', () => {
    game.throw(2)
    game.throw(8)
    const prevRound = game.getRound()
    expect(game.getState()).toBe(SRARE)
...
test('前一轮两投投中10个，这一轮第一次投球打中10个： 状态由SRARE变为STRIKE, 总分39,轮数+1', () => {
    game.throw(2)
    game.throw(8)
    const prevRound = game.getRound()
    expect(game.getState()).toBe(SRARE)
