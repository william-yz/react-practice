...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由STRIKE变为ONCE, 得分加16分', () => {
    const game = new Game()
    game.throw(10)
    expect(game.getState()).toBe(STRIKE)
    game.throw(8)
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由STRIKE变为ONCE, 得分加16分； 第二次投中2个， 状态由ONCE变为SRARE, 得分再加4分,轮数+1', () => {
    const game = new Game()
    game.throw(10)
    expect(game.getRound()).toBe(2)
    expect(game.getState()).toBe(STRIKE)
...
test('前一轮两投投中10个，这一轮第一次投球打中8个： 状态由STRIKE变为ONCE, 得分加16分； 第二次投中1个， 状态由ONCE变为NONE, 得分再加2分,轮数+1', () => {
    const game = new Game()
    game.throw(10)
    expect(game.getRound()).toBe(2)
    expect(game.getState()).toBe(STRIKE)
...
test('前一轮两投投中10个，这一轮第一次投球打中10个： 状态由STRIKE变为STRIKE, 得分加20分,轮数+1', () => {
    const game = new Game()
    game.throw(10)
    expect(game.getRound()).toBe(2)
    expect(game.getState()).toBe(STRIKE)
